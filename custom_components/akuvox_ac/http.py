...
def _inject_signed_paths(
    ...
    if signed:
        try:
            ...
            payload = "{}"
    try:
        api_paths_payload = json.dumps(SIGNED_API_PATHS)
    except Exception:  # pragma: no cover - shouldn't happen
        api_paths_payload = "{}"
    ...
    script = (
        ...
        "(function(){"
        "  try {"
        "    const apiPaths = Object.freeze(%s);"
        "    const apiTarget = Object.assign({}, window.AK_AC_API_PATHS || {}, apiPaths);"
        "    window.AK_AC_API_PATHS = apiTarget;"
        ...
        "    try { localStorage.removeItem('akuvox_hassTokens'); } catch (err) {}"
        "  } catch (err) {}"
        "  function akAcWindows(){"
        "    const out = [];"
        "    const add = function(win){ if (win && out.indexOf(win) === -1) out.push(win); };"
        "    try { add(window); } catch (err) {}"
        "    try { if (window.parent && window.parent !== window) add(window.parent); } catch (err) {}"
        "    try { if (window.top && window.top !== window) add(window.top); } catch (err) {}"
        "    try { if (window.opener) add(window.opener); } catch (err) {}"
        "    return out;"
        "  }"
        "  function akAcConnectionCandidates(win){"
        "    const out = [];"
        "    const add = function(value){ if (value && out.indexOf(value) === -1) out.push(value); };"
        "    try { add(win.hassConnection); } catch (err) {}"
        "    try { add(win.__HASS_CONNECTION__); } catch (err) {}"
        "    try {"
        "      const root = win.document && win.document.querySelector && win.document.querySelector('home-assistant');"
        "      const hass = root && (root.hass || root._hass);"
        "      if (hass) { add(hass.connection); add(hass.conn); }"
        "    } catch (err) {}"
        "    return out;"
        "  }"
        "  async function akAcResolveConnection(){"
        "    const wins = akAcWindows();"
        "    for (const win of wins) {"
        "      const candidates = akAcConnectionCandidates(win);"
        "      for (const candidate of candidates) {"
        "        try {"
        "          const resolved = candidate && typeof candidate.then === 'function' ? await candidate : candidate;"
        "          const holders = [resolved, resolved && resolved.conn, resolved && resolved.connection];"
        "          for (const holder of holders) {"
        "            if (holder && typeof holder.sendMessagePromise === 'function') return holder;"
        "          }"
        "        } catch (err) {}"
        "      }"
        "    }"
        ...
        "    return '';"
        "  }"
        "  function akAcRememberSignedPath(key, value){"
        "    if (!key || !value) return;"
        "    try {"
        ...
        "    if (!path || /[?&]authSig=/.test(path)) return path;"
        "    const conn = await akAcResolveConnection();"
        "    if (!conn) return path;"
        "    const response = await conn.sendMessagePromise({"
        "      type: 'auth/sign_path',"
        "      path: path,"
        "      expires: Number(expires || 43200)"
        "    });"
        "    const signed = response && (response.path || response.url || response);"
        "    if (typeof signed === 'string' && signed) {"
        "      akAcRememberSignedPath(akAcPathKey(path), signed);"
        "      return signed;"
        "    }"
        "    return path;"
        "  };"
        ...
        "  }"
        "  function akAcSignedPathExpiryMillis(value){"
        "    try {"
        "      const parsed = akAcParseUrl(value);"
        "      const sig = parsed && parsed.searchParams.get('authSig');"
        "      if (!sig) return 0;"
        "      const body = String(sig).split('.')[1] || '';"
        ...
        "    } catch (err) { return 0; }"
        "  }"
        "  function akAcSignedPathFresh(value, skewSeconds){"
        "    const exp = akAcSignedPathExpiryMillis(value);"
        "    if (!exp) return true;"
        "    return exp > Date.now() + Number(skewSeconds || 0) * 1000;"
        "  }"
        "  function akAcCacheBustPath(path){"
        "    try {"
        "      const parsed = new URL(path, window.location.origin);"
        "      parsed.searchParams.set('_akts', String(Date.now()));"
        ...
        "    let signed = await window.AK_AC_SIGN_URL(akAcCacheBustPath(sessionPath), 300);"
        "    if (!signed || !/[?&]authSig=/.test(signed)) throw new Error('authentication required');"
        "    if (!akAcSignedPathFresh(signed, 15)) {"
        "      signed = await window.AK_AC_SIGN_URL(akAcCacheBustPath(sessionPath), 300);"
        "    }"
        "    if (!akAcSignedPathFresh(signed, 15)) throw new Error('authentication signature expired');"
        ...
        "    return response;"
        "  }"
        "  window.AK_AC_GET_DASHBOARD_TOKEN = akAcDashboardToken;"
        "  window.AK_AC_DASHBOARD_FETCH = async function(input, options){"
